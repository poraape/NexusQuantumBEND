...
export const exportToXlsx = async (report: AuditReport, filename: string) => {
    ...
    const docDetailsData = report.documents.flatMap(d => {
        if (!d.doc.data || d.doc.data.length === 0) {
            // [CORREÇÃO DE TIPO] Adicionada 'Classificação' para garantir uma forma de objeto consistente, resolvendo o erro de tipo.
            return [{ 
        ...
        return d.doc.data.map(item => ({
            Documento: d.doc.name,
            Status: d.status,
            // FIX: The original code returned `undefined` for this property, while the other branch of the `flatMap`
            // returned 'N/A', causing a type mismatch for TypeScript's inference. Using 'N/A' as a fallback
            // in both cases ensures a consistent object shape and resolves the error.
            Classificação: d.classification?.operationType || 'N/A',
            ...item
        }));
    });
